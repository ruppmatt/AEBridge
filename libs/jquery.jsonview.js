...
Derrived from github.com/yesmeck/jquery-jsonview
 */
...
(function(jQuery) {
  ...
  JSONFormatter = (function() {
    ...
    JSONFormatter.prototype.arrayToHTML = function(array, level) {
      var collapsible, hasContents, index, numProps, output, value, _i, _len, retval;
      if (level == null) {
        level = 0;
      }
      hasContents = false;
      ...
      collapsible = (level === 0 || !hasContents) ? '' : ' collapsible';
      retval = "<ul class=\"array level" + level + collapsible + "\">";
      retval += (hasContents) ? output : "";
      retval += "</ul>";
      return retval;
    };

    JSONFormatter.prototype.objectToHTML = function(object, level) {
      var collapsible, hasContents, key, numProps, output, prop, value, retval;
      if (level == null) {
        level = 0;
      }
      hasContents = false;
      ...
      for (prop in object) {
        ...
        output += "<li><span class=\"prop\">" + key + "</span> " + (this.valueToHTML(value, level + 1));
        output += '</li>';
        numProps--;
      }
      collapsible = (level === 0 || !hasContents) ? '' : ' collapsible';
      retval = "<ul class=\"obj level" + level + collapsible + "\">";
      retval += (hasContents) ? output : "";
      retval += "</ul>";
      return retval;
    };
